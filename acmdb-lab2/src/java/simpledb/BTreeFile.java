...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		BTreeInternalPage curPage = (BTreeInternalPage)
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		BTreePageId RP = page.getRightSiblingId();

		rightPage.setRightSiblingId(RP);
		rightPage.setLeftSiblingId(page.getId());
		page.setRightSiblingId(rightPage.getId());
		if(RP != null) {
			BTreeLeafPage RPage = (BTreeLeafPage) getPage(tid, dirtypages, RP, Permissions.READ_WRITE);
			RPage.setLeftSiblingId(rightPage.getId());
			dirtypages.put(RP, RPage);
		}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
			BTreeInternalPage page, Field field)
					throws DbException, IOException, TransactionAbortedException {
		BTreeInternalPage rP = (BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
		...
		while(numEntriesToMove > 0) {
			...
			rP.insertEntry(entry);
			numEntriesToMove--;
		}
		BTreeEntry middleEntry = iter.next();
		page.deleteKeyAndRightChild(middleEntry);

		middleEntry.setLeftChild(page.getId());
		middleEntry.setRightChild(rP.getId());
		...
		page.setParentId(parent.getId());
		rP.setParentId(parent.getId());

		updateParentPointers(tid, dirtypages, rP);

		dirtypages.put(parent.getId(), parent);
		dirtypages.put(page.getId(), page);
		dirtypages.put(rP.getId(), rP);
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		for(int i=0;i<MoveNum;i++) {
			BTreeEntry lEntry = leftSibling.reverseIterator().next();
			BTreeEntry rEntry = page.iterator().next();
			Field middleKey = parentEntry.getKey();

			parentEntry.setKey(lEntry.getKey());
			leftSibling.deleteKeyAndRightChild(lEntry);
			page.insertEntry(new BTreeEntry(
					middleKey,
					lEntry.getRightChild(),
					rEntry.getLeftChild()));
		}
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		for(int i=0;i<MoveNum;i++) {
			BTreeEntry REntry = rightSibling.iterator().next();
			BTreeEntry LEntry = page.reverseIterator().next();
			Field middleKey = parentEntry.getKey();

			parentEntry.setKey(REntry.getKey());
			rightSibling.deleteKeyAndLeftChild(REntry);
			page.insertEntry(new BTreeEntry(
					middleKey,
					LEntry.getRightChild(),
					REntry.getLeftChild()));
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int MoveNum = rightPage.getNumTuples();
		for(int i=0;i<MoveNum;i++) {
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int MoveNum = rightPage.getNumEntries();
		BTreeEntry REntry;
		for(int i=0;i<MoveNum;i++) {
			REntry = rightPage.iterator().next();
			rightPage.deleteKeyAndLeftChild(REntry);
			leftPage.insertEntry(REntry);
		}
